...
const MovieDetailsPage = () => {
    ...
    const {
        data,
        error
    } = useSWR(tmdbAPI.getMovieDetail(moviesId), fetcher)
    if (!data) return []
            ...
            <div className="w-full h-[600px] relative">
                ...
                <div className="w-full h-[400px] max-w-[800px] mx-auto -mt-[200px] relative z-10 pb-10">
                    <img className="w-full h-full object-cover rounded-xl"
                         src={tmdbAPI.imageOriginal(poster_path)} alt=""/>
                </div>
    ...
    function MoviesCredits() {
        const {
            data,
            error
        } = useSWR(tmdbAPI.getMovieMeta(moviesId, "credits"), fetcher)
    ...
    function MovieVideos() {
        ...
        const {data, error} = useSWR(
            tmdbAPI.getMovieMeta(moviesId, "videos"), fetcher
        )
    ...
    function MovieSimilar() {
        const {moviesId} = useParams()
        const {data, error} = useSWR(
            tmdbAPI.getMovieMeta(moviesId, "similar"), fetcher
        )
        if (!data) return null
        const {results} = data
        if (!results || results.length <= 0) return null
